...
H2MatrixSolver::H2MatrixSolver(const MatrixGenerator& matgen, double epi, long long rank, long long leveled_rank, const std::vector<Cell>& cells, double theta, long long levels, double omega, double scale, MPI_Comm world) : 
  levels(levels), A(levels + 1), local_bodies(0, 0) {
  ...
  A[levels].construct(matgen, fix_rank ? (double)rank_func(levels) : epi, cells.data(), Near, comm[levels], A[levels], comm[levels], omega, scale);
  ...
  for (long long l = levels - 1; l >= 0; l--) {
    std::cout<<"Level "<<l<<std::endl;
    A[l].construct(matgen, fix_rank ? (double)rank_func(l) : epi, cells.data(), Near, comm[l], A[l + 1], comm[l + 1], omega, scale);
  }
