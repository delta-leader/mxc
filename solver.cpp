...
template <typename DT>
void H2MatrixSolver<DT>::factorizeDeviceM(int device, const cublasComputeType_t COMP) {
  std::cout<<"Compute type not supported for this dattype"<<std::endl;
  factorizeDeviceM(device);
}

template <>
void H2MatrixSolver<float>::factorizeDeviceM(int device, const cublasComputeType_t COMP) {
  long long dims_max = 0, lenA = 0, lenQ = 0;
  for (long long l = levels; l >= 0; l--) {
    dims_max = std::max(dims_max, *std::max_element(A[l].Dims.begin(), A[l].Dims.end()));
    lenA = std::max(lenA, (long long)A[l].ACols.size());
