...
void get_bounds(const elastWave3d::nodal_point* nodes, long long num_nodes, double R[], double C[]) {
  double Xmin[3], Xmax[3];
  for (int i = 0; i < 3; i++) {
    auto minmax = std::minmax_element(&nodes[0], &nodes[num_nodes], 
      [=](const elastWave3d::nodal_point& x, const elastWave3d::nodal_point& y) { return x.xc[i] < y.xc[i]; });
...
void buildBinaryTreeElems(Cell* cells, elastWave3d::element* elems, long long num_elems, long long levels, long long first_cell_idx) {
  cells[first_cell_idx].Body[0] = 0; //bodies_offset;
  cells[first_cell_idx].Body[1] = num_elems; //bodies_offset + nbodies;
  ...
  for (long long level = 0; level < levels; ++level) {
    ...
    for (long long i = 0; i < (1 << level); i++) {
      ...
      Cell& ci = cells[cell_offset + i];
      long long sdim = std::distance(ci.R.begin(), std::max_element(ci.R.begin(), ci.R.end()));
      long long elems_begin = ci.Body[0];// - bodies_offset;
      long long elems_end = ci.Body[1];// - bodies_offset;

      //long long i_num = i_end - i_begin;
      std::sort(&elems[elems_begin], &elems[elems_end], 
        [&](elastWave3d::element x, elastWave3d::element y) { return x.xc[sdim] < y.xc[sdim]; });
      ...
      //std::cout<<nodes_begin<<" "<<nodes_mid<<" "<<nodes_end<<std::endl;
      child0.Body[0] = elems_begin;// + bodies_offset;
      child0.Body[1] = elems_mid;// + bodies_offset;
      child0.nodes = false;
      child1.Body[0] = elems_mid;//+ bodies_offset;
      child1.Body[1] = elems_end;// + bodies_offset;
      child1.nodes = false;

      get_bounds(&elems[elems_begin], elems_mid - elems_begin, child0.R.data(), child0.C.data());
      get_bounds(&elems[elems_mid], elems_end - elems_mid, child1.R.data(), child1.C.data());
    }
  }
}
