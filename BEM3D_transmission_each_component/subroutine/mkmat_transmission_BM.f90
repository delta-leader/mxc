subroutine mkmat_transmission_BM(Cmat,rhs,omega,alpha)
   !test function is piecewise linear for domain 1.
   !test function is piecewise constant for domain 2.
   ...
   real(kind(0d0))::vec(3),dten2(3,3),mu1,mu2
...
!==============================================================
   ...
   !--- incident wave ---
   do ex=1,nel
      call freq_inc_displacement(el(ex),omega,zten2)
      do ip=1,3
         do i=1,3
            rhs(3*(el(ex)%ind(ip)-1)+i)=&
               &rhs(3*(el(ex)%ind(ip)-1)+i)+zten2(i,ip)
         end do
      end do
      ...
      do ip=1,3
         do i=1,3
            rhs(3*(el(ex)%ind(ip)-1)+i)=&
               &rhs(3*(el(ex)%ind(ip)-1)+i)+zten2(i,ip)*alpha/mu1
         end do
      end do
   end do
   !--- influence functions ---
   ...
   !--- for domain 1 ---
   !--------------------
   !$OMP parallel do private(sing,ex,ey,ip,iq,vec,dten2,zten2)
   do ix=1,nnode
      do nip=1,node(ix)%nel
         ...
         do iy=1,nnode
            do niq=1,node(iy)%nel
               ...
               if(ex == ey)then
                  sing=1
                  call mass_mat_ll(el(ex),dten2)
                  do i=1,3
                     Cmat(3*(ix-1)+i,3*(iy-1)+i)&
                        &=Cmat(3*(ix-1)+i,3*(iy-1)+i)&
                        &+0.5d0*dten2(ip,iq)
                  end do
               else
               ...
               !--- Tij ---
               call linear_x_Tij_freq_ip_iq(omega,zten2,el(ex),el(ey),sing,ip,iq)
               do i=1,3
                  do j=1,3
         ...
         do ey=1,nel
            ...
            if(ex == ey)then
               sing=1
               call mass_mat_lc(el(ex),vec)
               do i=1,3
                  Cmat(3*(ix-1)+i,nnode3+3*(ey-1)+i)&
                     &=Cmat(3*(ix-1)+i,nnode3+3*(ey-1)+i)&
                     &+0.5d0*vec(iq)*alpha
               end do
            else
            ...
            !--- Uij ---
            call linear_x_Uij_freq_ip(omega,zten2,el(ex),el(ey),sing,ip)
            do i=1,3
               do j=1,3
   ...
   mu2=mu
   !$OMP parallel do private(sing,ey,iq,vec,zten2)
   do ex=1,nel
      !--- calculation of Cmat ---
      do iy=1,nnode
         do niq=1,node(iy)%nel
