subroutine static_Sigma_d_Uij(xx_in,y1,y2,y3,on_id,sigma_ijk,sigma_ijk_d,Uij)
   !modified 2023.10.25
   use math_cst
   use elast_parameter
   implicit none
...
!--- singular part of fundamental solution sigma_ijk_d -----------
   ...
   do m=1,3
      do i=1,3
         do j=1,3
            sigma_ijk(m,i,j)=cst*(&
               &(1.0d0-2.0d0*nu)*(delta(m,i)*rir(j)+delta(m,j)*rir(i)-delta(i,j)*rir(m))&
               &+3.0d0*rijkr(m,i,j) )
            do iq=1,3
               do d=1,3
...
!--- singular part of fundamental solution Uij ---
   do iq=1,3
      ...
      do i=1,3
         do j=1,3
            Uij(i,j,iq)=((3.0d0-4.0d0*nu)*int1*delta(i,j)+int2(i,j))&
               &*1.0d0/(16.0d0*pi*mu*(1.0d0-nu))
         end do
      end do
   end do
