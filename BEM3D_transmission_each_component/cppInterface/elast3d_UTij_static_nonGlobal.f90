module elast3d_UTij_static_nonGlobal_mod
  implicit none
contains
  !----------------------------------------
  subroutine elast3d_UTij_static_nonGlobal(xx_in,y1,y2,y3,on_id,nvec,ust,tst,elastp)
    ...
    !----------------------------------------------------
    !----------------------------------------------------
    cst1=1.d0/(8.d0*pi*(1.d0-elastp%nu))
    cst2=1.d0-2.d0*elastp%nu
    !--- tst ---
    do i=1,3
       do j=1,3
          do iq=1,3
    ...
    do i=1,3
       do j=1,3
          ...
          do iq=1,3
             tst(i,j,iq)=-cst1*(&
                  &cst2*( elastp%delta(i,j)*(ay(iq)*eta*Ixz(3,2)+by(iq)*eta*Ixz(3,3)+cy(iq)*Ixz(3,1))-rinjq(i,j,iq)+rinjq(j,i,iq) )&
    ...
    !--- ust ---
    cst1=1.d0/(16.d0*pi*(1.d0-elastp%nu))
    cst2=3.d0-4.d0*elastp%nu
    do i=1,3
       do j=1,3
          ...
          ust(i,j)=cst1*(cst2*Ixz(1,1)*elastp%delta(i,j)+rirj)
       end do
    end do
