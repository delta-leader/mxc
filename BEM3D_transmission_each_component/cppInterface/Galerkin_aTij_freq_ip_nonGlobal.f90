module galerkin_atij_3d_entrywise_mod
  use iso_c_binding
  implicit none
contains
  !-------------------------------------
  subroutine linear_x_aTij_freq_ip_nonGlobal(x_nodals, xNumNodeBasis, elx, y_nodals, yNumNodeBasis, ely, omega, elastp, sing, ip, zaTij)
    !\int \phi_{ip} \int aTij dSy dSx
    use struct_type_fixed_len_node_mod
    use BEM3d_small_mod
    use math_cst
    use elast_parameter_struct_mod
    use elast3d_atij_entrywise_mod
    implicit none
  ...
  end subroutine linear_x_aTij_freq_ip_nonGlobal
  !-------------------------------------------
end module galerkin_atij_3d_entrywise_mod