module Dyn_Sigma_Uij_nonGlobal_mod
  implicit none
contains
!--------------------------------------------
subroutine elastUij_Sigma_dynamic_nonGlobal(x,y,cl,ct,zs,duij,dsigma,elastp)
   ...
   do i=1,3
      do j=1,3
         duij(i,j)=zcst*( zu_part(1)*r_y(i)*r_y(j)-zu_part(2)*elastp%delta(i,j) )/elastp%mu!not non-dimensionalization
         do k=1,3
            dsigma(i,j,k)=zcst*( zs_part(1)*(elastp%delta(i,j)*r_y(k)+elastp%delta(i,k)*r_y(j))&
               &+zs_part(2)*r_y(i)*elastp%delta(j,k)+zs_part(3)*r_y(i)*r_y(j)*r_y(k) )
         end do
      end do
   end do
 end subroutine elastUij_Sigma_dynamic_nonGlobal
!=============================================================================
!=============================================================================
!=============================================================================
 subroutine elastSigma_dynamic_nonGlobal(x,y,cl,ct,zs,dsigma,elastp)
   use math_cst, only: pi_4
   use elast_parameter_struct_mod
   implicit none
   integer::i,j,k
 ...
 end subroutine elastSigma_dynamic_nonGlobal
 !!=============================================================================
 !!===========================================================================
 !!===========================================================================
...
!---------------------------------
 end module Dyn_Sigma_Uij_nonGlobal_mod
