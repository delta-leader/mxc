module static_Sigma_d_Uij_nonGlobal_mod
  implicit none
contains
  !--------------------------------------------
  subroutine static_Sigma_d_Uij_nonGlobal(xx_in,y1,y2,y3,on_id,sigma_ijk,sigma_ijk_d,Uij,elastp)
    ...
    real(kind(0d0))::prho(2),rir(3),rijkr(3,3,3),phi3(3,3,3),int1,int2(3,3)
    ...
    real(kind(0d0)),dimension(3),intent(in)::xx_in,y1,y2,y3
    real(kind(0d0)),dimension(3,3,3),intent(out)::Uij,sigma_ijk
    real(kind(0d0)),dimension(3,3,3,3,3),intent(out)::sigma_ijk_d
    ...
    !----------------------------------------------------
    !----------------------------------------------------
    !--- singular part of fundamental solution sigma_ijk_d -----------
    do i=1,3
       rir(i)=Ixz(3,2)*h_phi(i,1)+Ixz(3,3)*h_phi(i,2)+Ixz(3,1)*h_phi(i,3)
    end do
    do i=1,3
       do j=1,3
          do k=1,3
             do l=1,3
                do m=1,3
                   do n=1,3
                      phi3(l,m,n)=h_phi(i,l)*h_phi(j,m)*h_phi(k,n)
                   end do
                end do
             end do
             rijkr(i,j,k)=Ixz(5,7)*phi3(1,1,1)+Ixz(5,10)*phi3(2,2,2)+Ixz(5,1)*phi3(3,3,3)&
                  &+Ixz(5,8)*(phi3(1,1,2)+phi3(1,2,1)+phi3(2,1,1))&
                  &+Ixz(5,4)*(phi3(1,1,3)+phi3(1,3,1)+phi3(3,1,1))&
                  &+Ixz(5,9)*(phi3(1,2,2)+phi3(2,1,2)+phi3(2,2,1))&
                  &+Ixz(5,6)*(phi3(2,2,3)+phi3(2,3,2)+phi3(3,2,2))&
                  &+Ixz(5,2)*(eta**2)*(phi3(1,3,3)+phi3(3,1,3)+phi3(3,3,1))&
                  &+Ixz(5,3)*(eta**2)*(phi3(2,3,3)+phi3(3,2,3)+phi3(3,3,2))&
                  &+Ixz(5,5)*eta*(phi3(1,2,3)+phi3(3,1,2)+phi3(2,3,1)+phi3(3,2,1)+phi3(1,3,2)+phi3(2,1,3))
          end do
       end do
    end do
    cst=-1.0d0/(8.0d0*pi*(1.0d0-elastp%nu))
    do m=1,3
       do i=1,3
          do j=1,3
             sigma_ijk(m,i,j)=cst*(&
                  &(1.0d0-2.0d0*elastp%nu)*(elastp%delta(m,i)*rir(j)+elastp%delta(m,j)*rir(i)-elastp%delta(i,j)*rir(m))&
                  &+3.0d0*rijkr(m,i,j) )
             do iq=1,3
                do d=1,3
                   sigma_ijk_d(m,i,j,d,iq)=sigma_ijk(m,i,j)&
                        &*(ay(iq)*ee1(d)+by(iq)*ee2(d))
                end do
             end do
          end do
       end do
    end do
    !--- singular part of fundamental solution Uij ---
    do iq=1,3
       int1=ay(iq)*Ixz(1,2)+by(iq)*Ixz(1,3)+cy(iq)*Ixz(1,1)
       do i=1,3
          do j=1,3
             do k=1,3
                do l=1,3
                   ph(k,l)=h_phi(i,k)*h_phi(j,l)
                end do
             end do
             int2(i,j)=Ixz(3,7)*ay(iq)*ph(1,1)&
                  &+Ixz(3,10)*by(iq)*ph(2,2)&
                  &+Ixz(3,1)*eta*cy(iq)*ph(3,3)&
                  &+Ixz(3,8)*(ay(iq)*(ph(1,2)+ph(2,1))+by(iq)*ph(1,1))&
                  &+Ixz(3,4)*(eta*ay(iq)*(ph(1,3)+ph(3,1))+cy(iq)*ph(1,1))&
                  &+Ixz(3,9)*(ay(iq)*ph(2,2)+by(iq)*(ph(1,2)+ph(2,1)))&
                  &+Ixz(3,6)*(eta*by(iq)*(ph(2,3)+ph(3,2))+cy(iq)*ph(2,2))&
                  &+Ixz(3,5)*(eta*(ay(iq)*(ph(2,3)+ph(3,2))+by(iq)*(ph(1,3)+ph(3,1)))+cy(iq)*(ph(1,2)+ph(2,1)))&
                  &+Ixz(3,2)*eta*(cy(iq)*(ph(1,3)+ph(3,1))+eta*ay(iq)*ph(3,3))&
                  &+Ixz(3,3)*eta*(cy(iq)*(ph(2,3)+ph(3,2))+eta*by(iq)*ph(3,3))
          end do
       end do
       do i=1,3
          do j=1,3
             Uij(i,j,iq)=((3.0d0-4.0d0*elastp%nu)*int1*elastp%delta(i,j)+int2(i,j))&
                  &*1.0d0/(16.0d0*pi*elastp%mu*(1.0d0-elastp%nu))
          end do
       end do
    end do
