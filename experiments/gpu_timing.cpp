...
int main(int argc, char* argv[]) {
  ...
  long long prec = argc > 5 ? std::atoll(argv[5]) : 0;
  const char* csv = argc > 6 ? argv[6] : nullptr;
  ...
  H2MatrixSolver<DT> matA(eval, 1e-6, rank, rank/2, cell, theta, &body[0], levels);
  ...
  H2MatrixSolver<DT> matM = H2MatrixSolver<DT>(eval, 0., rank, rank/2, cell, theta, &body[0], levels);
  ...
  for (int i=0; i<RUNS; ++i){
    if (prec > 0) {
      ...
      matM_low.factorizeDeviceM(handle, comp[prec]);
      matM_low.free_gpu_handles();
      matM_low.free_all_comms();
    } else {
      H2MatrixSolver<DT> matM2(matM);
      initNcclComms(&nccl_comms, matM2.allocedComm);
      matM2.init_gpu_handles(nccl_comms);
      matM2.factorizeDeviceM(handle, CUBLAS_COMPUTE_64F);
      matM2.free_gpu_handles();
      matM2.free_all_comms();
    }
  //H2MatrixSolver<DT_low> matM_low(matM);
