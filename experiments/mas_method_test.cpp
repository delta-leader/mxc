...
int main(int argc, char* argv[]) {
  ...
  Laplace3D<DT> eval(1.);
  //Yukawa3D<DT> eval(1, 5.);
  //Gaussian<DT> eval(1);
  //Helmholtz3D<DT> eval(1., 5.);
  ...
  { // without using a factorization basis
    // HSS construction
    MPI_Barrier(MPI_COMM_WORLD);
    double hss_construct_time = MPI_Wtime(), hss_construct_comm_time;
    H2MatrixSolver hss(eval, epi, rank, cell, 0, &body[0], levels, true);
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    H2MatrixSolver h2(eval, epi, rank, cell, theta, &body[0], levels, true);
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    Vector_dt<DT> vhss(lenX);
    Vector_dt<DT> vh2(lenX);
    Vector_dt<DT> ref(lenX);
    // copy the random vector
    std::copy(&Xbody[hss.local_bodies.first], &Xbody[hss.local_bodies.second], &vhss[0]);
    ...
    mat_vec_reference(eval, lenX, Nbody, &ref[0], &Xbody[0], &body[hss.local_bodies.first * 3], &body[0]);
    refmatvec_time = MPI_Wtime() - refmatvec_time;
    // calculate relative error between H-matvec and dense matvec
    double hss_err = computeRelErr(lenX, &vhss[0], &ref[0]);
    ...
    std::copy(ref.begin(), ref.end(), vhss.begin());
    ...
    h2.solvePrecondition(&vh2[0]);
    MPI_Barrier(MPI_COMM_WORLD);
    h2_sub_time = MPI_Wtime() - h2_sub_time;
    h2_sub_comm_time = ColCommMPI::get_comm_time();
    h2_err = computeRelErr(lenX, &vh2[0], &Xbody[hss.local_bodies.first]);
    Vector_dt<DT> Rh2(lenX);
    mat_vec_reference(eval, lenX, Nbody, &Rh2[0], &vh2[0], &body[hss.local_bodies.first * 3], &body[0]);
    double h2_res = computeRelErr(lenX, &Rh2[0], &ref[0]);
    ...
    hss.solvePrecondition(&vhss[0]);
    MPI_Barrier(MPI_COMM_WORLD);
    hss_sub_time = MPI_Wtime() - hss_sub_time;
    hss_sub_comm_time = ColCommMPI::get_comm_time();
    hss_err = computeRelErr(lenX, &vhss[0], &Xbody[hss.local_bodies.first]);
    Vector_dt<DT> Rhss(lenX);
    mat_vec_reference(eval, lenX, Nbody, &Rhss[0], &vhss[0], &body[hss.local_bodies.first * 3], &body[0]);
    double hss_res = computeRelErr(lenX, &Rhss[0], &ref[0]);
    ...
    if (mpi_rank == 0) {
      ...
      std::cout << "Residual HSS (A): " << hss_res << std::endl << std::endl;
      ...
      std::cout << "Residual H2 (A): " << h2_res << std::endl;
    }
    h2.free_all_comms();
    hss.free_all_comms();
  }*/

  std::cout<<std::endl<<"Factorization Basis"<<std::endl;
  std::cout<<std::endl<<"Fixed Rank = "<<(fixed_rank ? "true" : "false")<<std::endl;
  { // without using a factorization basis
    // HSS construction
    MPI_Barrier(MPI_COMM_WORLD);
    double hss_construct_time = MPI_Wtime(), hss_construct_comm_time;
    H2MatrixSolver hss(eval, epi, rank, cell, 0, &body[0], levels, fixed_rank, true);
    MPI_Barrier(MPI_COMM_WORLD);
    hss_construct_time = MPI_Wtime() - hss_construct_time;
    hss_construct_comm_time = ColCommMPI::get_comm_time();
    ...
    double h2_construct_time = MPI_Wtime(), h2_construct_comm_time;
    // create the H2 matrix
    H2MatrixSolver h2(eval, epi, rank, cell, theta, &body[0], levels, fixed_rank, true);
    MPI_Barrier(MPI_COMM_WORLD);
    h2_construct_time = MPI_Wtime() - h2_construct_time;
    h2_construct_comm_time = ColCommMPI::get_comm_time();
