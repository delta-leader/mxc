...
int main(int argc, char* argv[]) {
  ...
  // plus one level for fusing the trees
  ...
  std::cout<<"Levels (node tree) "<<levels_nodes<<std::endl;
  long long Nleaf_nodes = (long long)1 << levels_nodes;
  std::cout<<"Nleaf (node tree) "<<Nleaf_nodes<<std::endl;
  long long ncells_nodes = Nleaf_nodes + Nleaf_nodes - 1;
  long long levels = levels_elems + 1;
  long long Nleaf = Nleaf_nodes + Nleaf_elems;
  long long ncells = ncells_elems + ncells_nodes + 2;
  ...
  //nodes.insert(nodes.end(), elems.begin(), elems.end());
  ...
  // I don't think we need the center and radius for the root?
