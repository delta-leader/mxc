...
void compute_factorize(const cublasComputeType_t COMP, cublasHandle_t cublasH, long long bdim, long long rank, long long D, long long M, long long N, const long long ARows[], const long long ACols[], std::complex<float>* A, std::complex<float>* R, const std::complex<float>* Q, const ColCommMPI& comm) {
  ...
  //cublasCgemmBatched(cublasH, CUBLAS_OP_C, CUBLAS_OP_T, bdim, bdim, bdim, &one, U, bdim, A_SS, bdim, &zero, B, bdim, M);
  cublasGemmBatchedEx(cublasH, CUBLAS_OP_C, CUBLAS_OP_T, bdim, bdim, bdim, reinterpret_cast<void*>(&one), reinterpret_cast<void**>(U), CUDA_C_32F, bdim, reinterpret_cast<void**>(A_SS), CUDA_C_32F, bdim, reinterpret_cast<void*>(&zero), reinterpret_cast<void**>(B), CUDA_C_32F, bdim, M, COMP, ALGO);
  //cublasCgemmBatched(cublasH, CUBLAS_OP_C, CUBLAS_OP_T, bdim, bdim, bdim, &one, U, bdim, B, bdim, &zero, A_SS, bdim, M);
  cublasGemmBatchedEx(cublasH, CUBLAS_OP_C, CUBLAS_OP_T, bdim, bdim, bdim, reinterpret_cast<void*>(&one), reinterpret_cast<void**>(U), CUDA_C_32F, bdim, reinterpret_cast<void**>(B), CUDA_C_32F, bdim, reinterpret_cast<void*>(&zero), reinterpret_cast<void**>(A_SS), CUDA_C_32F, bdim, M, COMP, ALGO); 
  ...
  if (0 < rank) {
    //cublasCgemmBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, rdim, rank, bdim, &one, V_R, bdim, B, bdim, &zero, A_RS, bdim, M);
    cublasGemmBatchedEx(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, rdim, rank, bdim, reinterpret_cast<void*>(&one), reinterpret_cast<void**>(V_R), CUDA_C_32F, bdim, reinterpret_cast<void**>(B), CUDA_C_32F, bdim, reinterpret_cast<void*>(&zero), reinterpret_cast<void**>(A_RS), CUDA_C_32F, bdim, M, COMP, ALGO);

    for (long long i = M; i < lenA; i += N) {
      long long len = std::min(lenA - i, N);
      //cublasCgemmBatched(cublasH, CUBLAS_OP_C, CUBLAS_OP_T, bdim, bdim, bdim, &one, &U[i], bdim, &A_SS[i], bdim, &zero, B, bdim, len);
      cublasGemmBatchedEx(cublasH, CUBLAS_OP_C, CUBLAS_OP_T, bdim, bdim, bdim, reinterpret_cast<void*>(&one), reinterpret_cast<void**>(&U[i]), CUDA_C_32F, bdim, reinterpret_cast<void**>(&A_SS[i]), CUDA_C_32F, bdim, reinterpret_cast<void*>(&zero), reinterpret_cast<void**>(B), CUDA_C_32F, bdim, len, COMP, ALGO);
      //cublasCgemmBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, bdim, bdim, bdim, &one, &V[i], bdim, B, bdim, &zero, &A_SS[i], bdim, len);
    ...
    thrust::for_each(thrust::cuda::par.on(stream), inc_iter, inc_iter + (rdim * rank * M), copyFunc(rdim, rank, const_cast<const cuComplex**>(A_RS), bdim, &B[D], bdim));
    //cublasCgemmBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, rdim, rdim, bdim, &one, V_R, bdim, U_R, bdim, &zero, B_I_Cols, bdim, M);
    cublasGemmBatchedEx(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, rdim, rdim, bdim, reinterpret_cast<void*>(&one), reinterpret_cast<void**>(V_R), CUDA_C_32F, bdim, reinterpret_cast<void**>(U_R), CUDA_C_32F, bdim, reinterpret_cast<void*>(&zero), reinterpret_cast<void**>(B_I_Cols), CUDA_C_32F, bdim, M, COMP, ALGO);
    cudaMemcpyAsync(&R[D * block], &Bdata[D * block], M * block * sizeof(cuComplex), cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
    ...
    if (M < lenA) {
      //cublasCgemmBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, rank, rank, rdim, &minus_one, &A_SR[M], bdim, &B_Cols[M], bdim, &one, &A_SS[M], bdim, lenA - M);
      cublasGemmBatchedEx(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, rank, rank, rdim, reinterpret_cast<void*>(&minus_one), reinterpret_cast<void**>(&A_SR[M]), CUDA_C_32F, bdim, reinterpret_cast<void**>(&B_Cols[M]), CUDA_C_32F, bdim, reinterpret_cast<void*>(&one), reinterpret_cast<void**>(&A_SS[M]), CUDA_C_32F, bdim, lenA-M, COMP, ALGO);
    }

    for (long long i = M; i < lenA; i += N) {
      long long len = std::min(lenA - i, N);
      //cublasCgemmBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, rdim, rank, rdim, &one, &B_I_Cols[i], bdim, &A_SR[i], bdim, &zero, B, bdim, len);
      cublasGemmBatchedEx(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, rdim, rank, rdim, reinterpret_cast<void*>(&one), reinterpret_cast<void**>(&B_I_Cols[i]), CUDA_C_32F, bdim, reinterpret_cast<void**>(&A_SR[i]), CUDA_C_32F, bdim, reinterpret_cast<void*>(&zero), reinterpret_cast<void**>(B), CUDA_C_32F, bdim, len, COMP, ALGO);
      cublasCgemmBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, rank, rank, rdim, &minus_one, &A_SR[i], bdim, B, bdim, &zero, &ACC[i], rank, len);
      cublasGemmBatchedEx(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, rank, rank, rdim, reinterpret_cast<void*>(&minus_one), reinterpret_cast<void**>(&A_SR[i]), CUDA_C_32F, bdim, reinterpret_cast<void**>(B), CUDA_C_32F, bdim, reinterpret_cast<void*>(&zero), reinterpret_cast<void**>(&ACC[i]), CUDA_C_32F, rank, len, COMP, ALGO);      
    }
