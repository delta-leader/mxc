...
public:
  ...
  H2MatrixSolver(const MatrixGenerator& matgen, double epi, long long rank, long long leveled_rank, const std::vector<Cell>& cells, double theta, long long levels, double omega, bool io = false, MPI_Comm world = MPI_COMM_WORLD);
  ...
  //void solveGMRESDevice(deviceHandle_t handle, double tol, H2MatrixSolver& M, DT X[], const DT B[], long long inner_iters, long long outer_iters, const ncclComms nccl_comms);
